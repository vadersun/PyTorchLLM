...
# set model paras
tokenizer = AutoTokenizer.from_pretrained("chatglm2-6b-hf", trust_remote_code=True)
model = AutoModel.from_pretrained("chatglm2-6b-hf", trust_remote_code=True).to(device).to(dtype)
model = model.eval()
...
elif str(args.input_tokens) in list(prompt_pool["chatglm"].keys()):
    print(prompt_pool["chatglm"][str(args.input_tokens)])
    prompt = prompt_pool["chatglm"][str(args.input_tokens)]
else:
    print(list(prompt_pool["chatglm"].keys()))
